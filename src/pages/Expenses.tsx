...
export default function ExpensesPage() {
    ...
    }, [expenseTransactions, period]);

    const totalExpenses = useMemo(() => filteredExpenses.reduce((sum, t) => sum + t.amount, 0), [filteredExpenses]);
    const averageExpense = useMemo(() => filteredExpenses.length === 0 ? 0 : totalExpenses / filteredExpenses.length, [totalExpenses, filteredExpenses]);
    ...
    }, [filteredExpenses, categories, getCategoryDisplayName, language]);
    ...
    }, [filteredExpenses, period, language]);

    const COLORS = ['#EF4444', '#F87171', '#B91C1C', '#991B1B', '#FECACA'];
        ...
        <div className="flex flex-col h-[calc(100vh-6rem)] gap-4">
            ...
            <div className="flex items-center justify-between shrink-0">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h1 className="text-xl font-bold text-text-light dark:text-text-dark">{language === 'tr' ? 'Gider Analizi' : 'Expense Analysis'}</h1>
                        <p className="text-sm text-text-secondary-light dark:text-text-secondary-dark">{language === 'tr' ? 'Harcamalarını detaylı incele' : 'Detailed expense overview'}</p>
                    </div>
                </div>
                <div className="flex bg-slate-100 dark:bg-slate-800 rounded-lg p-1">
                    {(['week', 'month', 'year'] as TimePeriod[]).map((p) => (
                        <button key={p} onClick={() => setPeriod(p)} className={`px-4 py-1.5 rounded-md text-sm font-medium transition-colors ${period === p ? 'bg-white dark:bg-surface-dark text-danger shadow-sm' : 'text-text-secondary-light dark:text-text-secondary-dark hover:text-text-light dark:hover:text-text-dark'}`}>
                            {periodLabels[p]}
                        </button>
                    ))}
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 shrink-0">
                <div className="bg-white dark:bg-surface-dark rounded-xl p-4 border border-slate-200 dark:border-slate-800 flex items-center justify-between">
                    <div>
                        <p className="text-sm text-text-secondary-light dark:text-text-secondary-dark mb-1">{language === 'tr' ? 'Toplam Gider' : 'Total Expense'}</p>
                        <p className="text-2xl font-bold text-danger">{formatAmount(totalExpenses)}</p>
                    </div>
                    <div className="p-3 bg-danger/10 rounded-full"><DollarSign className="w-5 h-5 text-danger" /></div>
                </div>
                <div className="bg-white dark:bg-surface-dark rounded-xl p-4 border border-slate-200 dark:border-slate-800 flex items-center justify-between">
                    <div>
                        <p className="text-sm text-text-secondary-light dark:text-text-secondary-dark mb-1">{language === 'tr' ? 'Ortalama' : 'Average'}</p>
                        <p className="text-2xl font-bold text-text-light dark:text-text-dark">{formatAmount(averageExpense)}</p>
                    </div>
                    <div className="p-3 bg-slate-100 dark:bg-slate-700 rounded-full"><TrendingDown className="w-5 h-5 text-text-secondary-light dark:text-text-secondary-dark" /></div>
                </div>
                <div className="bg-white dark:bg-surface-dark rounded-xl p-4 border border-slate-200 dark:border-slate-800 flex items-center justify-between">
                    <div>
                        <p className="text-sm text-text-secondary-light dark:text-text-secondary-dark mb-1">{language === 'tr' ? 'İşlem Adedi' : 'Count'}</p>
                        <p className="text-2xl font-bold text-text-light dark:text-text-dark">{filteredExpenses.length}</p>
                    </div>
            ...
            <div className="flex-1 min-h-0 grid grid-cols-1 lg:grid-cols-3 gap-4">
                ...
                <div className="lg:col-span-2 bg-white dark:bg-surface-dark rounded-xl border border-slate-200 dark:border-slate-800 p-4 flex flex-col">
                    <h3 className="text-base font-semibold text-text-light dark:text-text-dark mb-4">{language === 'tr' ? 'Gider Trendi' : 'Expense Trend'}</h3>
                        ...
                        {trendData.length > 0 ? (
                            <ResponsiveContainer width="100%" height="100%">
                                <AreaChart data={trendData} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
                                    ...
                                    <Tooltip
                                        formatter={(value: number) => [formatAmount(value), language === 'tr' ? 'Gider' : 'Expense']}
                                        contentStyle={{ backgroundColor: '#1E293B', border: 'none', borderRadius: '8px', color: '#fff' }}
                                    />
                                    <Area type="monotone" dataKey="amount" stroke="#EF4444" strokeWidth={3} fill="url(#expenseGradient)" activeDot={{ r: 6, strokeWidth: 0 }} />
                                </AreaChart>
                            </ResponsiveContainer>
                        ) : (
                            <div className="h-full flex items-center justify-center text-text-secondary-light dark:text-text-secondary-dark">
                                {language === 'tr' ? 'Bu dönemde işlem yok' : 'No transactions in this period'}
                            </div>
                        )}
                    </div>
                </div>
